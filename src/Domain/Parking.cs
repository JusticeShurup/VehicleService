namespace Domain
{
    public class Parking
    {
        ...
        {
            var parkingPlace = ParkingPlaces.Where(p => p.Vehicle == null).FirstOrDefault();
            if (parkingPlace == null)
            {
                throw new Exception("There are no free places");
            }
            if (parkingPlace.Vehicle != null)
            {
                parkingPlace.Vehicle.ParkingPlace = null;
            }
            parkingPlace.Vehicle = vehicle;
            vehicle.ParkingPlace = parkingPlace;
        }
    }
}
