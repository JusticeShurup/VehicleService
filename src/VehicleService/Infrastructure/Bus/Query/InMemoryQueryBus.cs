...
namespace Infrastructure.Bus.Query
{
    public class InMemoryQueryBus : IQueryBus
    {
        ...
        public async Task<TResponse> Send<TResponse>(Application.Base.Query.Query query)
        {
            var handler = GetWrappedHandlers<TResponse>(query);

            if (handler == null) throw new Exception("Query is not registered");

            return await handler.Handle(query, _provider);
        }

        private QueryHandlerWrapper<TResponse> GetWrappedHandlers<TResponse>(Application.Base.Query.Query query)
        {
